...
int IOU::TrackData2Rect_OriginalBBox(tracking_module_data_t *tracking_data,
  ...
  list_for_each_safe(pos, next, &tracking_data->active_targets[0]) {
    ...
    HOBOT_CHECK(idx < input_num)
        << "index:" << idx << " input num:" << input_num
        << " track count:" << track_cnt;

    // check if the state of idx is default
    HOBOT_CHECK(!updated[idx]) << "index:" << idx << " input num:" << input_num
                               << " track count:" << track_cnt;

    if (DataState::VALID == in_rects.at(idx)->state_) updated[idx] = true;
...
std::shared_ptr<IOUParam> IOU::GetConfig() {
  auto select_config = std::static_pointer_cast<IOUParam>(config_param_);
